...
int main(int argc, char** argv) {
    ...
    MPI_Dist_graph_neighbors(graph_communicator, indegree, sources, sources_weight, outdegree, destinations, destinations_weight);

    bool global_done = false;
    int32_t distance_to_source = (source == Rank) ? 0 : std::numeric_limits<int32_t>::max();
